...
class ArduinoSerialControl(Node):
    ...
    def __init__(self):
        ...
        self.steer_rad_max = self.get_parameter("steer_rad_max").get_parameter_value().double_value
        ...
        self.steer_pwm_min = self.steer_pwm_center - self.steer_pwm_gap
        self.steer_pwm_max = self.steer_pwm_center + self.steer_pwm_gap
        ...
        self.log_filename = "src/data/arduino_log" + now.strftime("_%m_%d_%H_%M_%S")
        if self.log_enable:
        ...
        # linear.x 값을 구독하는 추가 구독자 생성
        self.linear_x_subscriber = self.create_subscription(
            Twist,
            'linear_x',  # 'linear_x' 토픽 구독
            self.linear_x_callback,
            10
        )
    ...
    # 구독한 linear.x 값을 로그로 출력
        ...
        self.get_logger().info(f'Subscribed linear.x: {self.v_curr}')
    ...
    def vel_sub_callback(self, msg):        
        ...
        self.get_logger().info(f'V: {self.v_ref}, W: {self.w_ref}')
    ...
    def control_loop(self):
        ...
        if self.v_ref < self.v_min:
            ...
            self.v_ctl = self.v_ref + self.p_gain*self.v_err+self.d_gain*self.dv_err
    ...
    def convert_ang_to_pwm(self, ang):
        f_ang = 2.49 * ang
        ctl_steer_pwm = self.ang_to_pwm_scale * f_ang + self.steer_pwm_min
